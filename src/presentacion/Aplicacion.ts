...
import { Tarea, PropiedadesTarea } from "../modelos/Tarea";
...
export class Aplicacion {
    ...
    private async procesarOpcionPrincipal(op: string): Promise<void> {
        switch (op) {
            case "1": await this.menuVerMisTareas(); break;
            case "2": await this.menuBuscarTarea(); break;
            case "3": await this.menuAgregarTarea(); break;
            case "4": await this.menuEstadisticas(); break;
            default: await this.pausa("Opción inválida.");
        }
    }

    private async menuVerMisTareas(): Promise<void> {
        let volver = false;
        while (!volver) {
            console.clear();
            ...
            switch (op) {
                case "1": lista = this.gestor.ordenarPorTitulo(); titulo = "Todas"; break;
                case "2": lista = this.gestor.filtrarPorEstado(Estado.PENDIENTE); titulo = "Pendientes"; break;
                case "3": lista = this.gestor.filtrarPorEstado(Estado.EN_CURSO); titulo = "En Curso"; break;
                case "4": lista = this.gestor.filtrarPorEstado(Estado.TERMINADA); titulo = "Terminadas"; break;
                case "5": lista = this.gestor.ordenarPorVencimiento(); titulo = "Por Vencimiento"; break;
                case "6": lista = this.gestor.ordenarPorCreacion(); titulo = "Por Creación"; break;
                case "7": lista = this.gestor.ordenarPorDificultad(); titulo = "Por Dificultad"; break;
                default: await this.pausa("Opción inválida."); continue;
            }
    ...
    private async listadoTareas(lista: Tarea[], titulo: string): Promise<void> {
        let volver = false;
        while (!volver) {
            console.clear();
            console.log(`${titulo}.\n`);
    ...
    private async menuDetallesTarea(tarea: Tarea): Promise<void> {
        let volver = false;
        while (!volver) {
            console.clear();
            ...
            if (op === "0") return;
            if (op === "E") { await this.menuEdicionTarea(tarea); return; }
            if (op === "D") { await this.eliminarTareaUI(tarea); return; }
            if (op === "R") { await this.verRelacionadas(tarea); }
        }
    }

    private mostrarDetalle(tarea: Tarea): void {
    ...
    private async eliminarTareaUI(tarea: Tarea): Promise<void> {
    ...
    private async verRelacionadas(tarea: Tarea): Promise<void> {
        const relacionadas = this.gestor.obtenerRelacionadas(tarea);
        console.log(`\n--- Relacionadas con "${tarea.titulo}" ---`);
        if (relacionadas.length === 0) console.log("Ninguna encontrada.");
        relacionadas.forEach(t => console.log(`- ${t.titulo}`));
        await this.pausa("");
    }

    private async menuEdicionTarea(tarea: Tarea): Promise<void> {
        console.clear();
        ...
        try {
            ...
            if (desc !== "") actualizaciones.descripcion = desc === " " ? null : desc;
            if (estStr !== "") {
                const e = Estado.desde(estStr);
                if (e) actualizaciones.estado = e;
            }
            if (difStr !== "") {
                const d = Dificultad.desde(difStr);
                if (d) actualizaciones.dificultad = d;
            }
            if (venStr !== "") {
    ...
    private async menuAgregarTarea(): Promise<void> {
        console.clear();
        ...
        try {
            const nueva = new Tarea({
                titulo: tit,
                descripcion: desc,
                estado: Estado.desde(estStr) || Estado.PENDIENTE,
                dificultad: Dificultad.desde(difStr) || Dificultad.FACIL,
    ...
    private async menuBuscarTarea(): Promise<void> {
        console.clear();
        const q = (await this.consola.preguntar("Buscar: ")).trim();
        if (!q) return;

        const res = this.gestor.buscarPorTitulo(q);
        if (res.length > 0) await this.listadoTareas(res, "Resultados");
        else await this.pausa("No se encontraron resultados.");
    }

    private async menuEstadisticas(): Promise<void> {
        console.clear();
    ...
    private async pausa(msg: string): Promise<void> {
        await this.consola.preguntar(msg ? `\n${msg}` : "\nPresiona Enter...");
    }

    // Helpers de UI
    private formatearFecha(d: Date | null): string {
        if (!d) return "Sin datos";
        const pad = (n: number) => (n < 10 ? "0" + n : n);
        return `${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())}`;
    }

    private parsearFecha(s: string): Date | null {
        const d = new Date(s);
