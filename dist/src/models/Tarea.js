...
class Tarea {
     ...
     */
    actualizar(datos) {
        const nuevosDatos = { ...datos }; // Shallow copy
        ...
        return new Tarea({
            ...
            ultimaEdicion: new Date() // Side-effect controlado: nueva fecha
        });
    }
    /**
     * Verifica si la tarea está vencida. (Pure)
     */
    estaVencida() {
        if (!this.vencimiento)
            return false;
    ...
    /**
     * Verifica si la tarea coincide con un término de búsqueda. (Pure)
     */
    ...
    /**
     * Verifica si es de alta prioridad. (Pure)
     */
    esPrioridadAlta() {
        return this.dificultad.code >= 3 && this.estado === Estado_1.Estado.PENDIENTE;
    }
    /**
     * Verifica si está relacionada con otra tarea. (Pure)
     */
